...
#import soft_renderer123.cuda.voxelization as voxelization_cuda
...
def voxelize_sub1(faces, size, dim):
    ...
    return sr.cuda.voxelization.voxelize_sub1(faces, voxels)[0].transpose(dim + 1, -1)
...
def voxelize_sub2(faces, size):
    ...
    nf = faces.size(1)
    voxels = torch.zeros(bs, size, size, size).int().cuda()
    return sr.cuda.voxelization.voxelize_sub2(faces, voxels)[0]
...
def voxelize_sub3(faces, voxels):
    ...
    voxels, visible = sr.cuda.voxelization.voxelize_sub3(faces, voxels, visible)
    #voxels, visible = voxelization_cuda.voxelize_sub3(faces, voxels, visible)

    sum_visible = visible.sum()

    while True:
        voxels, visible = sr.cuda.voxelization.voxelize_sub4(faces, voxels, visible)
